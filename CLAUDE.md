...
- **`036-write-path-hardening` delivered** 2026-09-05 — the nine concurrency-and-abuse failure modes audited against the delivered write path, and the five that were open closed together (669 tests). A raced tag attach and an EF-detected rowversion mismatch answer `409` instead of `500`; a deadlock victim answers **`503 errors/transient-conflict`** with `Retry-After`; a **general write limit** — 60 per caller per minute, configurable — joins `004b`'s sign-in throttle without replacing it; and `POST /api/tickets` accepts an **`Idempotency-Key`**. **Four of the five sections were wrong until they were RUN**, and the most valuable is `036` §3.3: **a deadlock never arrives as a `DbUpdateException`** — EF's `SqlServerExecutionStrategy` rethrows it inside an `InvalidOperationException` — so the translation matched nothing while the code read as though it worked. Match the inner CHAIN, never the wrapper's type
- **`036b-transient-read-path` delivered** 2026-09-05 — `036` translated a deadlock at `SaveChangesAsync`, which covers the WRITE and nothing else; a victim chosen on a `SELECT` was still a `500`. **One MediatR behaviour, `TransientFailureBehaviour`, registered OUTERMOST and deliberately UNCONSTRAINED** — the pipeline is `TransientFailure → Validation → Transaction → Audit` now, and `PipelineOrderTests` was updated in the same change rather than loosened (672 tests). **Not four wrappers on `IApplicationDbContext`'s read methods:** `TicketTimelineQuery` calls EF directly on DbSets and `SequenceTicketNumberGenerator` uses `SqlQueryRaw`, so four wrappers leave two holes — `003b`'s rejected per-table grant in another costume
- **A control that was expected to be a formality DID NOT FAIL, and that is the finding** 2026-09-05 — `036b`'s spec claimed that registering the behaviour inside `AuditBehaviour` would make BR-9's failure row record `transient-conflict` instead of the fault. It was run. **Nothing went red**: `AuditOutcomeClassifier` maps any non-denial `DomainException` to `Failed`, which is exactly what the raw engine exception maps to. The claim had already been written into a spec, two code comments and a test's remarks; **all four were corrected in place with the disproof kept beside them**. What survives is one argument — a behaviour inside `TransactionBehaviour` has already returned when the `COMMIT` runs — and it is **unproven**, because a COMMIT-time deadlock could not be induced. **Nothing goes red if someone moves that line**, and the files say so
- **A pure query cannot be a deadlock victim, so `036b` AC-4 is recorded NOT MET** 2026-09-05 — under READ COMMITTED a `SELECT` releases its shared lock immediately, so a read-only session holds nothing another could wait on: it can be blocked forever and never chosen. Inducing one would mean adding `HOLDLOCK` to the query under test, which is changing the thing being measured. The honest assertion took its place — the behaviour is *present* in a query's pipeline — and the criterion is recorded unmet in the shape `008` AC-3 uses
- **A limit sized for a person is not sized for the test suite, and the failures land somewhere else** 2026-09-05 — `036`'s first full run failed **174 tests** on a fixed 60-writes-per-minute limit, because the integration suite drives every write in the product as two seeded users inside one minute. The red tests were about *language* and *assignees* — `400 expected, 429 found` — so it read as those features breaking. **The limit is `RateLimit:WritesPerWindow` now**, raised in `WaslApiFactory` and lowered to five by the limiter's own tests through a second host built with `WithWebHostBuilder`. The alternative was weakening a production protection to fit a test host
- **Two latent defects that only a new feature's data could surface** 2026-09-05 — `TagReadTests` compared SQL collation order with `StringComparer.Ordinal` while its own comment said *"under the database collation"*; `dbo.Tags.Name` is case-insensitive, so the two agreed only while every tag in the database shared a case, and `036`'s lowercase tags broke the coincidence. **The `ORDER BY` was never wrong — the comparer was.** And adding `503` to `ProblemTypes` turned `ProblemRegistryTests` red *as designed*, which then revealed that `405`, `415` and `429` were in the test's list and **absent from the documented table** — the "second, independent statement" had become one statement plus a copy
- **`037-icon-system` delivered** 2026-09-05 — the product owner supplied a 63-icon document, and *"add these icons so they are shared"* turned out to be a request to **replace** the set: it draws a 20-unit keyline against the house 16, `stroke-width` 1.75 at 16px against a constant 1.5, and a **filled node** — its own stated signature, on 7 icons — against `icons.md`'s *"Fills: None."* Both readings went to the product owner before any code. Ruled **full adopt, house rules win**, so every filled circle is a stroked ring and **the document's signature does not ship**; what carries the set is still the tighter keyline and the derived radius. **One** module, `src/wasl-web/src/icons/icons.tsx`, 73 exports — 62 from the document plus 11 labelled (D) — with four guards and 338 tests (1000 frontend tests total). `icons-added.tsx` is deleted
- **A rule with no guard is a preference, and `icons.md` Rule 2 had been one** 2026-09-05 — `037` built the keyline check *before* any icon and ran it against the set as it stood: **9 of 39 icons were outside the 16-unit keyline** with a green build — `IconAssign` `IconClosed` `IconSms` `IconFilter` `IconCopy` `IconAlert` `IconTriangleAlert` `IconCircleInfo` by 0.5–1.0 units and `IconWhatsapp` by 0.081. Rule 2 is one of the two rules the set's whole signature rests on. **It also found `IconEye` declared in BOTH icon files** with different geometry (`r 2.4` / `r 2.5`), so `TicketListPage.tsx` and `Input.tsx` had been rendering different drawings under one import name. **And `icons-added.tsx`'s stated reason for existing was already false** — it claimed `icons.tsx` was a byte-for-byte copy protected by a drift check; `026` had already changed `IconFilter` and appended four icons, and no drift check ever existed
- **The measuring tool produced a well-formed report about nothing** 2026-09-05 — **SVG elliptical-arc flags are single characters and may be glued to the number after them**: `a2.5 2.5 0 003.5 6` is `rx ry rot laf sf x y`, seven arguments, and a plain number regex reads `003.5` as one token and mis-slots every coordinate after it. It reported `copy` starting at x = 6 when it starts at 3.5 and missed two icons entirely, and **nothing errored**. The rewrite carries **four controls whose answers are known by hand, run before any real figure is read**, and throws rather than returning an empty box — `008`'s query-counter rule. **A second claim in the same spec was reasoned rather than measured and was also wrong:** `IconWhatsapp`'s 0.08 overhang was called sampling noise, and solving its arc centre in closed form gave (12.081, 11.919) — real ink. **Sampling can only under-report a curve's extreme, never invent one**, so "the tool is imprecise" was never available for an overhang it *found*
- **A negative control that cannot fail for the reason it was written** 2026-09-05 — `037`'s C3 appended a second `export const IconEye` to prove the duplicate-name guard, and the run reported `Test Files 1 failed · Tests no tests`: **a duplicate `const` in one file is a compile error**, so the module never loaded and the assertion never ran. That assertion is unfalsifiable — the compiler gets there first — and **the load-bearing half is the cross-file scan**, which is the shape the real defect had. C3b plants a stray icon in another file and goes red naming it. Recorded rather than counted as a passing control
- **Directional icons mirror by ONE rule now, and nineteen had been missing it** 2026-09-05 — the document lists 20 glyphs that flip under RTL; the repo did it per consumer and **exactly one consumer ever did** (`.signOut .rowIcon`). An arrow pointing the wrong way in Arabic reads as a rendering bug, not a missing rule, so nobody files it. `data-flip` on the icon, one rule in `styles/base.css`. **`037` AC-8 said the flip set must equal the document's list "no more, no fewer", and that was refined rather than obeyed** — a strict reading deletes `IconSignOut`'s flip, and the document cannot speak to an icon it does not contain; two literals now, each asserted exact. **`refresh` and `reopen` mirroring overrides an argument we had written down** — a rotation points *around* the reading axis, not along it — and the override is recorded in the module, not dropped
- **Judge at the size that ships, and magnify the RASTER, not the vector** 2026-09-05 — `037`'s `settings` became a slider (three full-width tracks, marks at three positions), and the open question was whether its r2 mark stays a **ring** at 18px or fills in and converges with `filter`. Scaling the SVG re-renders the vector and answers a different question, so each icon was rasterised at its true size and the raster magnified ×10 with smoothing off. **At 18px the hole survives**; at 16px it is nearly gone. Two findings beyond the question asked: the two glyphs **would not have converged even on the losing branch**, because full-width tracks against centred bars is a bigger difference than the "only bar length" premise assumed — and **16px is the size to watch**
- **`026-ticket-list` + `027-ticket-detail` — the two ticket screens, v3** 2026-09-01 — 511 frontend tests. `/tickets` with the `Table` primitive, chip counts, applied-filter chips, a numbered pager and a created-date range; **`/tickets/mine` and `/tickets/unassigned` are the SAME screen with `assignee` decided by the PATH** — never in the URL, no removable chip, survives `مسح الكل`, absent from the filter badge, and not counted as filtering so an empty personal queue is not "no matches". `/tickets/:id` rebuilt to the v3 canvas: the **status pill IS the control**, two tabs over `034`'s `?type=`, newest-first four-at-a-time, a 292px rail (assignee picker · customer + their other tickets via `?customerId=` · five facts), five derived tag tints, and an amber Send in internal mode
- **The rule that shaped `027`, and it is a rule for every screen after it** 2026-09-01 — build the columns the backend has; anything with no counterpart is absent from the design. **Refined the same day: draw the unbuilt ACTIONS and leave them read-only.** The two are not in tension — a menu row promises nothing until it is pressed, so an inert row with a stated reason is a roadmap the screen carries; a **data** region is different, because a countdown drawn from nothing is a fact the product does not have and **looks exactly like a working one**. So escalate · merge · extend-due are inert **with no client fetcher at all** (a test asserts it: a `disabled` prop is one edit away from deletion, a function that does not exist is not), while the SLA pill, the rail SLA block, the breach banner, `@mentions` and the priority-change history row are **absent**
- **The timeline's pages arrive ASCENDING, and the flip belongs per page** 2026-09-01 — the SQL orders `OccurredAtUtc DESC` and the handler returns the newest N **oldest-first** (`013` Q-2's chat order, and `013`'s frozen contract says so in words). v3 labels the feed «الأحدث أولاً», so the client reverses **each page** — reversing the flattened list puts the second page's rows ahead of the first page's, with every row present and every count right. **Only a two-page test can fail on that**; a single-page test passes either way
- **`position: sticky` establishes a stacking context, and an inline `style` beats the stylesheet** 2026-09-01 — two defects from one session, both invisible to jsdom. The assignee panel's `z-index: 200` ranked it *inside* the sticky rail, so the composer's `position: relative` wrappers painted through it. And the sidebar grew to the content's height because of `min-block-size: 100vh` in a flex row — **the fix was inert** for its first attempt, because `Sidebar.tsx` carried `style={{ position: 'relative' }}`: the module said `sticky`, computed style said `relative`, nothing errored. `shellLayout.test.ts` is a source scan for exactly that, because no rendered test in jsdom can see a cascade being lost
- **Colour the backend does not carry is DERIVED from the name, never invented** 2026-09-01 — `TagSummary` is `(id, name)` and `SupportUserOption` is `(id, fullName, role)`. Five hues, and three corrections all from the screen: three buckets collide by arithmetic over five tags; six buckets from a five-hue palette produce two washes nobody can tell apart; and **summing code units clusters on Arabic names**, which share letters, so two of three seeded agents matched at four buckets *and* at five. FNV-1a with `Math.imul` spreads them. **A tag must differ from the tag beside it (a collision walks within the ticket); a person must be one colour everywhere (no walk)** — de-colliding per region would give one person two colours on one screen
- **A guard can pass, or fail, on its own prose** 2026-09-01 — the absence guard for `027`'s unbacked regions searched for `sla` and went red on `useTranslation`; the sidebar guard then went red on the comment *explaining* the declaration it forbids. Both strip comments before scanning, and each carries a control proving the stripper ran. **And `tokens.css` said `--on-brand` is "COMPUTED at runtime" when nothing computes it** — that sentence was the leading hypothesis for an hour. Corrected in place
- **`027`'s v2 preview is DELETED, and a preview is not a design of record** 2026-09-01 — it rendered the superseded screen from the same stylesheet the real page uses, so every shared class silently restyled a design nobody was maintaining. The CSS it was the only consumer of went with it, pruned mechanically with all 86 removed selectors printed for review. `docs/sdd/design/screens/04-ticket-detail.md` is the v3 design of record now, with both earlier revision tables kept. **`027` AC-11 — "the preview is reviewed before anything is wired" — is recorded VOID**, not dropped
- **Architecture audit — the layering holds, and two of the three findings were wrong** 2026-09-07 — measured, not read: `Wasl.Domain` has zero packages and zero framework usings; `Wasl.Application` references EF and ASP.NET nowhere outside doc comments; all eight controllers dispatch through one `ISender` and name no `WaslDbContext`; 15 handlers sit in `Features/` folders and the two in `Infrastructure/Queries` are the pair `CLAUDE.md` sanctions by name. **Placement cleanup**: `DashboardTargets` and `OrganizationTimeZone` moved from the Infrastructure ROOT into `Queries/`, beside their only consumer, the same move 2026-08-29 made for `JwtOptions`. **The `DateTime.UtcNow` in `DemoSeeder` is gone** — the last inline call in `src/` — and the comment beside it was found to be false: it said the three raw INSERTs were "three INSERTs that `007` deletes", and `007` shipped without deleting them. Corrected in place. **Two findings did not survive being checked.** "The tag commands have no validator" is not a gap: both ids come from the ROUTE (`{id:guid}/tags/{tagId:guid}`), and route ids answer `404` product-wide — `GetTicketByIdQuery` has no validator either, and `008` AC-3 records that behaviour. And "`Features/Tickets/Tags/` breaks one-folder-per-use-case" is real but splitting it makes it worse: `TagSummary` is shared vocabulary — `CreateTicketResult` returns a list of them — so the folder survives a split anyway and the result is one shared folder plus three thin ones. **The reason is written into `TagContracts.cs` instead**, which is what the working agreement asks for: a deviation is fine, an undocumented one is not
