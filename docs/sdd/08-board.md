...
| | **`005b-language-preference`** | **US-014** | **✅ Backend half done 2026-08-30** — 533 tests, 0 warnings, 12 new, all green on the first run. `PUT /api/me/language`, `SupportUser.ChangeLanguage` (the entity's **first mutator**), and **no migration** — `004` shipped the column and the claim, `005` shipped the provider that reads it. **Split before it started**: the switcher screen is the frontend lane's own row. **The limitation is the headline and it is written in three places:** a user who switches sees no change until the next sign-in, because the token is signed and immutable — and the `204` carries `Content-Language` naming the OLD language, which the frozen contract calls *"the single most confusing thing about this endpoint"* and which AC-6 now asserts so it cannot be filed as a defect. **`002c`'s two-way contract comparison was seen to fail for the first time here**: changing the route to `api/mine` fired BOTH directions at once. Same language twice is `204` not `409` (a preference is not a state machine), and there is no `expectedVersion` (nobody races themselves). **Open and named: the numbering.** The product owner calls this `005b`; the frozen contract lives under `014` |
| | **`005b-language-preference`** | **US-014** | **NEW, named 2026-08-29 by the product owner** — the two pieces of `005`'s original scope that need a user-facing surface: **`PUT /api/me/language`** with the `PreferredLanguage` column and its migration, and **the language switcher screen**. Split out rather than deferred inside `005`'s spec, and the reason is explicit: *a feature that crosses the lane boundary makes both lanes wait for each other, and that costs more than two features do.* `005` reads the language claim and falls through when it is absent; **`005b` is where that claim gets a real producer.** Sits between `005` and `014` — `014` is the manual Arabic walk of every screen, which needs the switcher to exist first |
