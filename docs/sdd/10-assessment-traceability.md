...
|---|---|
| **Requirement & Specification** (10) | **`15-scope-coverage.md` first** — every one of the twelve sections of the supplied scope document traced to a status, with the source document's silence on deliverables and acceptance criteria recorded rather than assumed away. Then `01-product-spec.md` (numbered FR/NFR, and which of them were *derived* rather than given), `04-business-rules.md` (BR-* as testable propositions), each story's `spec.md`, and `11-open-questions.md` for the questions raised rather than guessed |
| **Planning & Task Breakdown** (20) | **`15-scope-coverage.md` and `16-three-day-plan.md` first** — the scope decision and the nine-hour schedule cut from it, each cut carrying a reason that would hold with more time. Then each story's `plan.md` (design, data changes, contract changes, rejected alternatives) and `tasks.md` (ordered, dependency-aware, individually verifiable, one owner each); `08-board.md` for the sequencing and the explicit deferral decisions |
