...
| ☑ | At least two real alternatives were considered and rejected with reasons | `plan.md` — four rejected, two of them genuinely attractive (the pipeline owning `SaveChanges`; one `sa` connection string) |
...
| ☑ | The silent failures each have their own criterion | AC-3 (filter dropped) · AC-8 (denial row rolled back with the thing that failed) · AC-10 (row lost to a cancelled token) · AC-13 (`DENY` on a principal it cannot restrict) · AC-14 (rule test with an empty population) · AC-18/AC-19 (diff read after `SaveChanges` accepted it) |
...
| ☑ | `DENY`, not `REVOKE` | ADR-013 row 10, BR-9.5 |
...
| ☑ | The audit row is written by a pipeline behaviour, not by each handler | Constitution V, ADR-010, `docs/sdd/02-architecture.md` |
| ☑ | One transaction per request, opened by a behaviour | ADR-010, *Supporting decisions* |
...
| ☑ | Behaviour file names match the blueprint's spelling | `docs/sdd/02-architecture.md` — `Behaviors/`, `AuditBehavior.cs`; `research.md` R-11 |
...
| Gap | Reason |
|---|---|
| The NFR-10 rule test has an **empty population** when this feature closes | `Wasl.Api` has no `ICommand` implementation until `004`. Handled rather than hidden: AC-14b runs the scanner over a deliberate violator, so the mechanism is proven even though the real set is empty. `research.md` R-5, assumption A-5 |
