...
| 4 | BR-9.2's `401` audit row is in *Rules referenced* | **Nothing writes an audit row on a `401`.** `004` AC-17/AC-18 are deferred to `004b` — a gap in BR-9.4 | `008` cannot satisfy that reference and does not claim to. Removed from the rules list with the reason. **Closed 2026-08-29 by `004b`:** the row is written now, by an `IAuthorizationMiddlewareResultHandler`, for every endpoint including these two. Nothing in `008` changed — the gap was never `008`'s to close, and it did not pretend otherwise |
