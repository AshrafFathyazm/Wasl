...
Seventeen backend features are delivered: `001` · `002` + `002b` + `002c` · `003` + `003b` · `004` + `004b` · `005` · `014` backend half · `007` · `008` ·
...
014-language-preference-and-rtl     backend half DONE 2026-08-30
                                    switcher + Arabic pass — the frontend lane's
026/027                             the frontend lane's
```
...
| ✅ | `014` backend half | **Done 2026-08-30.** 533 tests. `PUT /api/me/language`, no migration — `004` shipped the column and the claim. **Split before it started**, so the switcher screen and the manual Arabic pass stay the frontend lane's. **Written as `005b` during the session that built it and merged into `014` on 2026-08-30 by ruling: the files win, `005b` was a name in conversation and never a folder in the repository** |
...
| `026-ticket-list` · `027-ticket-detail` · `028-generated-api-types` · `029-loader-system` · `030-feedback-layer` · `031-frontend-dropdown` | — | The frontend lane's. **Not indexed here individually, and that is a gap in this file rather than in the work** — each has its own folder. `029` and `031` are delivered; `028` is blocked pending authorisation; **`030` is delivered in part** — 2026-09-05: toast, its host and modal are built and wired, and `design/feedback-layer.md` is the design of record. **The side panel's four variants are not built**, so AC-11, AC-12, AC-13 and AC-17 are unmet and AC-10 needs a ruling. See its `summary.md` §6 |
| `039-ticket-row-actions` | — | **✅ Delivered 2026-09-06.** Reassign and Close act from the ticket ROW now, each with a toast. **This reverses `026`'s "every item navigates" ruling and answers its objection rather than dismissing it** — the row still has no `version` and no `allowedTransitions` (`010` excluded both on purpose), so opening either surface FETCHES the ticket under the detail route's own key: one request, three answers, no rule implemented twice. Reassign is a 308px menu with a JS-measured height cap and an upward flip; Close is a `Modal` composing four reasons, a duplicate reference and a summary into the ONE 500-character `note` the contract carries. +68 tests (1147 in 49 files), nine negative controls. **The mock-up was RUN first and four of its eight findings changed the build** — its «إشعار العميل» checkbox is `display:none` and outside the dialog, it closes an `InProgress` ticket that BR-1 forbids, and its own upward flip is unreachable at its page height. Four rulings before any code; three things it draws are ABSENT because `021` and `Interaction` are unbuilt. `AssigneePanel` and the tinted `Avatar` moved out of `TicketDetailPage` so both screens render one component. **One defect shipped and was reported from the running app in ENGLISH** — see the delivery log |
| `038-new-ticket-redesign` | — | **✅ Delivered 2026-09-06.** `/tickets/new` rebuilt from a supplied HTML mock-up and six named changes: no conditional disabling, two columns at ≥1100px, channel as five radio cells, a customer card with company, **duplicate detection**, and an error summary in a fixed footer. +33 tests (1063 in 44 files). **The mock-up was RUN first and three of its behaviours are defects** — its two-column layout never renders (inline `style` beats its own media query, `027`'s `Sidebar.tsx` defect exactly), its duplicate banner shows no count, and its summary and focus disagree. **That third one then appeared in our build via React Hook Form's `shouldFocusError` default and the test caught it on its first run.** Seven rulings before any code; five mock-up decisions overridden, and **R-7 reversed the same day** so `src/icons/` is untouched and `037`'s trademark question stays open. First client to send an `Idempotency-Key`. **Walked in both languages 2026-09-06** — every paint criterion MET; the rail measures 316px on the inline-start at 1443 and stacks below 1100, and R-4b was seen working on a customer with 0 open and 2 closed tickets. Only AC-36 remains unmet |
| `037-icon-system` | — | **✅ Delivered 2026-09-05.** The product owner supplied a 63-icon document; `add these icons so they are shared` turned out to be a request to REPLACE the set, and both readings were put to them before any code. Ruled **full adopt, house rules win** — the document's metaphors and geometry, normalised to `icons.md`'s 16-unit keyline, constant 1.5 stroke and no fills, so **the document's own signature (the filled node, 7 icons) does not ship**. **One** module of 73 exports; `icons-added.tsx` deleted and its stated reason for existing found to have been false since `026`. Four guards, 338 tests, each broken on purpose. **Found that `icons.md` Rule 2 had drifted — 9 of 39 icons outside the keyline with a green build — and that `IconEye` was declared in BOTH files with different geometry.** AC-13 partially met: the source HTML arrived mis-decoded and only its ASCII half could be vendored honestly |
