    ...
    : IAccessTokenIssuer
{
    public (string Token, DateTime ExpiresAtUtc) Issue(SupportUser user)
    {
        ...
        Claim[] claims =
        [
            new(WaslJwtClaimNames.Sub, user.Id.ToString()),
            new(WaslJwtClaimNames.Email, user.Email),
            new(WaslJwtClaimNames.Role, user.Role.ToString()),
            new(WaslJwtClaimNames.PreferredLanguage, user.PreferredLanguage),
            ...
            new(WaslJwtClaimNames.Jti, Guid.CreateVersion7().ToString()),
